...
const Add: React.FC<Props> = ({ handleAddItemToCart, handleBuyNow, hanldeFavorite, onQuantityChange }) => {
  ...
  function handleQuantity(type: 'i' | 'd'): void {
    ...
    onQuantityChange(newQuantity); // Gọi callback để truyền giá trị mới lên cha
  }
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center gap-4">
        <div className="bg-gray-100 py-2 px-4 flex items-center justify-between w-32">
          <button onClick={() => handleQuantity('d')} className="cursor-pointer text-xl">
            -
          </button>
          {quantity}
          <button onClick={() => handleQuantity('i')} className="cursor-pointer text-xl">
            +
          </button>
        </div>
